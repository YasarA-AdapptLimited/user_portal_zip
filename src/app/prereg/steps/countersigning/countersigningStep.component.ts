...
export class CountersigningStepComponent extends FormStepComponent {
  ...
  showHelp = false;
  stepId = RegApplicationStep.Countersigning;
  ...
  constructor(service: FormStepperService, private preregService: PreregService,
  ...
  sendForCountersigning() {
    ...
    dialogRef.afterClosed().subscribe(confirmed => {
      if (confirmed) {
        this.serverErrors = undefined;
        this.sending = true;
        this.preregService.sendToCountersigner(this.selectedPharmacist.registrationNumber).subscribe(() => {
          ...
          setTimeout(() => {
            ...
            this.service.disableAllStepsExcept(RegApplicationStep.Countersigning);
          }, 50);
  ...
  recall() {
    this.recalling = true;
    this.preregService.recallFromCountersigner().subscribe(() => {
      this.recalling = false;
      this.application.activeForm.formStatus = ApplicationStatus.InProgress;
      this.service.setStepRange(1, RegApplicationStep.Countersigning);
