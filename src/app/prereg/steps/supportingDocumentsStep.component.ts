...
import { FinalTrainingReport } from '../model/FinalTrainingReport';
...
export class SupportingDocumentsStepComponent extends FormStepComponent implements OnInit {
  ...
  title = 'Documents';
  stepId = RegApplicationStep.SupportingDocuments;
  hasProofOfId = false;
  ...
  FinalTrainingReport = FinalTrainingReport;
  registrantStatus: RegistrantStatus;
  isFinalTrainingReportPass = false;
  ...
  ngOnInit() {
    ...
    // this.requireQualificationCertificate = !this.authService.user.isSandwichStudent;
    // if (!this.requireQualificationCertificate) {
    //   this.docTypes = 6;
    // }
  ...
  get isTrainee() {
    return this.registrantStatus == RegistrantStatus.PreRegistrationTrainee;
  }

  removeFinalDeclarationDocsFromSupportingDocs(element) {
    return element.type === AttachmentType.FinalDeclarationForm;
  }
  load() {
    if (this.viewReady) {
