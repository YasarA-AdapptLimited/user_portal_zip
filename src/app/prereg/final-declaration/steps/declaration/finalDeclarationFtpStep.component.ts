...
@Component({
  selector: 'app-final-declaration-ftp-step',
  templateUrl: './finalDeclarationFtpStep.component.html',
  providers: [
    {
      provide: FormStepComponent,
      useExisting: forwardRef(() => FinalDeclarationFtpStepComponent)
    },
    FormValidationService,
    { provide: FormValidator, useExisting: FormValidationService }
  ]
}) export class FinalDeclarationFtpStepComponent extends FormStepComponent implements OnInit {
  declarationHeading;
  ...
  dynamicFormId;
  valid;
  constructor(private finalDeclarationService: FinalDeclarationService, private validator: FormValidationService, service: FormStepperService) {
    super(service);
  ...
  ngOnInit() {
    ...
    if (this.formIndex === 0) {
      this.stepId = FinalDeclarationStep.FtpDeclaration1;
      this.declarationHeading = 'Provisional registration: declaration of fitness to practise';
    }
    if (this.formIndex === 1) {
      this.stepId = FinalDeclarationStep.FtpDeclaration2;
