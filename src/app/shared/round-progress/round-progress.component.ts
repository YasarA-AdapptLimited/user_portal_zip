import { Component, Input, Output, OnChanges, NgZone, EventEmitter, ViewChild, Renderer2 } from '@angular/core';
...
export class RoundProgressComponent implements OnChanges {
  ...
  private _setPath(value: number): void {
    if (this._path) {
      __ngRendererSetElementAttributeHelper(this._renderer, this._path.nativeElement, 'd', this._service.getArc(value, this.max, this.radius - this.stroke / 2, this.radius, this.semicircle));
    }
  }
