...
import { StudentActivation } from './model/StudentActivation';
...
@Component({
  moduleId: module.id,
  selector: 'app-student-activation',
  templateUrl: './studentActivation.component.html',
...
export class StudentActivationComponent implements OnInit {
  ...
  loading = false;
  saving = false;
  @Input() activation: StudentActivation;
  ...
  ngOnInit() {
    this.courseTypes = Object.keys(CourseType)
      .filter(key => typeof CourseType[key] !== 'number' && key !== '717750007')
      .map(key => ({
        value: CourseType[key], key: parseInt(key, 10)
      }));
  ...
  activate() {
    ...
    this.service.verifyStudent(this.activation).subscribe(data => {
  ...
  activateStudent() {
    ...
    this.service.activateStudent(this.activation).catch(response => {
        setTimeout(() => {
          this.loading = false;
          this.activationFailed = true;
