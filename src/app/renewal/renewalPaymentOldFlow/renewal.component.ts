...
export class RenewalOldComponent implements OnInit, CanComponentDeactivate {
  ...
  allowNext() {
    ...
    if (this.currentStep.payment) {
      ...
      if (this.renewal.paymentMethod === RenewalPaymentMethod.PaymentCard) {
        return !!this.renewal.selectedCard && this.renewal.agreed;
      } else {
  ...
  saveDeclaration() {
    ...
    this.tracking.postActivity(UserActivity.Renewal, 'Declaration save (end of flow)').subscribe(() => {
      this.service.saveDeclarationOld(payload).subscribe(cartId => {
        this.saving = false;
        this.step++;
        // TODO: window should be abstracted
        window.scrollTo(0, 0);
  ...
  saveDeclarationAndPaymentSelection() {
    ...
    this.tracking.postActivity(UserActivity.Renewal, 'Declaration save & worldpay redirect (end of flow)').subscribe(() => {
      this.service.saveDeclarationOld(payload).subscribe(cartId => {
        this.selectedCard = this.renewal.selectedCard;
        this.cartId = JSON.stringify(cartId);
        this.redirectToWorldpay();
      }, error => {
        this.handleValidationErrors(error);
        this.saving = false;
      });
